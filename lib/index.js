 ...
 * @property {string|null}              [getter=getLocale]              - Getter method in request object to get current locale.
 * @property {string|null}              [setter=setLocale]              - Setter method in request object to set current locale.
...
const optionsSchema = {
    ...
    translations: Joi.any().allow(null).default(defaultOptions.translations),
    ...
    interpolation: Joi.object({prefix: Joi.string().required(), suffix: Joi.string().required()}).optional().allow(null).default(defaultOptions.interpolation),
};
...
Internal.prototype.bestMatch = function bestMatch(requested) {
    ...
    for (let one of requested) {
        if (this.locales.indexOf(one.substr(0,2)) > -1) return one.substr(0,2);
    }
};
 ...
 * @private
 */
Internal.prototype.getTranslators = function getTranslators() {
    let translators = {};

    if (typeof this.options.translations === "function") {
        this.options.translations
            .keys()
            .forEach(
                (file)=>{
                    let locale = path.basename(file, path.extname(file));
                    // console.log("////////////////////////////////")
                    // console.log(locale, ":", this.options.translations(file));
                    translators[locale] = new Polyglot(
                        {
                            locale: locale,
                            phrases: this.options.translations(file),
                            interpolation: this.options.hasOwnProperty("interpolation") ? this.options.interpolation: undefined
                        }
                    );
                }
            );
        ...
        for (let key in this.options.translations) {
            if (this.options.translations.hasOwnProperty(key)) {
                console.log("////////////////////////////////")
                console.log("interpolation", this.options.hasOwnProperty("interpolation") ? this.options.interpolation: undefined)
                translators[key] = new Polyglot({
                    locale: key,
                    phrases: this.options.translations[key],
                    interpolation: this.options.hasOwnProperty("interpolation") ? this.options.interpolation: undefined
                });
            }
        }
...
Internal.prototype.getAvailableLocales = function getAvailableLocales() {
    ...
    locales = locales.concat(lodash.keys(this.translators));
    ...
    // Locale files
    if (locales.length === 0 && this.options.scan && this.options.scan.path) {
        locales = this.scan();
    }
...
Internal.prototype.processRequest = function processRequest(request, h) {
    let locale = this.options.default;
    try {
    ...
    console.log(`this.translators[${locale}]`, this.translators.hasOwnProperty(locale));
    request.i18n = this.translators[locale];
